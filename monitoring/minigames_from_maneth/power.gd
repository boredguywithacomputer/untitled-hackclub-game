...
func _process(delta: float) -> void: # returns the player to the 3D map once the power minigame is done
	if globalvars.powerDone == "yes":
		globalvars.container.remove_child(globalvars.power)
